...
//function for jcomboreturn a list of all owners in a hashmap
    public HashMap<String, Integer> getOwnersMap()
    {
        ...
        try {
            st = Connect_to_DB.getTheConnection().createStatement();
            rs = st.executeQuery("SELECT * FROM `primary_db`");
            
            DB_Primary owners;
            
            while(rs.next())
            {
                owners = new DB_Primary(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4));
    ...
    public DB_Primary findResident (int ownerId)
    {
        ...
        String searchQuery = "SELECT * FROM `primary_db` WHERE `id`=?";
